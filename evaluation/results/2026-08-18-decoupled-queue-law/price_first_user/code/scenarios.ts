import {
  createRandom,
  standardNormalFor,
} from "../../../../../code/simulation/random.js";
import {
  REQUEST_COUNT,
...
export function buildScenarioTrace(args: {
  ...
  if (tasks.length === 0 || models.length === 0) {
    throw new Error("Trace inputs cannot be empty");
  }
  const random = createRandom(
    `quality-first-arrival:${definition.id}:${definition.seed}`,
  );
  const horizonMs =
    definition.requestCount / definition.arrivalRatePerSecond * 1_000;
  const requests = Array.from(
    { length: definition.requestCount },
    (_, index): ExperimentRequest => {
      ...
      return {
        ...
        baseTtftByModel: Object.fromEntries(models.map((model) => [
          model.id,
          naturalTtft(model, definition.id, definition.seed, index),
        ])),
      };
    },
  );
...
export function cloneTraceForMethods<const Method extends string>(
  trace: readonly ExperimentRequest[],
  methods: readonly Method[],
): Record<Method, ExperimentRequest[]> {
  ...
  ])) as Record<Method, ExperimentRequest[]>;
}
